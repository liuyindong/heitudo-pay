...
Page({
  ...
  getShopTime:function(){
    ...
    //获取我的订单
    wx.request({
      url: app.globalData.apiHost+'/shop-times/',
      method: 'GET',
      data: {},
      header: {
      ...
      success: function (res) {
        var openTime = res.data.openTime
  ...
  getReduction: function () {
    ...
    //获取我的订单
    wx.request({
      url: app.globalData.apiHost+'/reductions/myreduction/'+wx.getStorageSync('openId'),
      method: 'GET',
      data: {},
      header: {
        'Accept': 'application/json'
      ...
      success: function (res) {
        ...
        let len = res.data.length > 2 ? 2 : res.data.length
        for (let i = 0; i < len;i++){
          if (res.data[i].isR == 0) {
            showAdStatus = true
          }
